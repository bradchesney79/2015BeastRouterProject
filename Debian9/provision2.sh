...
apt install -y curl wget apt-transport-https apt-src
...
echo "#wget https://nginx.org/keys/nginx_signing.key && apt-key add nginx_signing.key" >> /etc/apt/sources.list
...
## Install Atheros & Ralink firmware

apt-get install -y firmware-realtek firmware-atheros firmware-misc-nonfree

## Install networking tools

apt-get install -y wireless-tools net-tools ifconfig iw wpa_supplicant bridge-utils ebtables 
...
pre-up bash /etc/network-clean-slate.sh

# The On-Board RTL Ethernet Port
auto enp7s0
...
# The 4 x Gigabit Intel Ethernet Card Ports
...
bridge_ports enp4s0f0 enp4s0f1 enp5s0f0 enp5s0f1 wlp1s0 wlp6s0
  address 192.168.1.220
  netmask 255.255.255.0
#  pre-up killall hostapd; killall wpa_supplicant
#  post-up hostapd -B /etc/hostapd/wlp1s0.conf /etc/hostapd/wlp6s0.conf
...
apt-src -y install hostapd

## "Thanks goes to Kelsar of the #debian channel on freenode irc for guidance on bridging the wifi to the existing ethernet bridge..."

## hostapd configuration for 
...
ht_capab=[SHORT-GI-20][SHORT-GI-40][TX-STBC][RX-STBC1][DSSS_CCK-40][LDPC][HT20]
#ht_capab=[SHORT-GI-20][SHORT-GI-40][TX-STBC][RX-STBC1][DSSS_CCK-40][LDPC][HT20][HT40]
#ht_capab=[SHORT-GI-20][SHORT-GI-40][TX-STBC][RX-STBC1][DSSS_CCK-40][LDPC][HT40-][HT40+]
EOL

cat > /etc/hostapd/wlp6s0.conf <<EOL
...
# service.
##
dhcp-range=192.168.1.235,192.168.1.240,12h
